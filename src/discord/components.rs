    ...
    if custom_id == "select_torrent" {
        if let ComponentInteractionDataKind::StringSelect { values } = &comp.data.kind {
            if let Some(hash) = values.first() {
                let _ =
                    views::show_torrent_details(handler, ctx, &interaction, user, hash, settings)
                        .await;
            }
        }
        ...
        for u in &settings.users {
            if u.role == "administrator"
                && u.discord_id.is_some()
                && u.discord_id.as_deref() != Some("9876543210123")
            {
                if let Some(ref admin_dc_id) = u.discord_id {
                    if let Ok(admin_user_id) = admin_dc_id.parse::<u64>() {
                        ...
                        {
                            let approve_admin = CreateButton::new(format!(
                                "dc_auth:approve:administrator:{}:{}",
                                comp.user.id, comp.user.name
                            ))
                            .label("Approve Admin")
                            .style(ButtonStyle::Success);
                            let approve_manager = CreateButton::new(format!(
                                "dc_auth:approve:manager:{}:{}",
                                comp.user.id, comp.user.name
                            ))
                            .label("Approve Manager")
                            .style(ButtonStyle::Primary);
                            let approve_reader = CreateButton::new(format!(
                                "dc_auth:approve:reader:{}:{}",
                                comp.user.id, comp.user.name
                            ))
                            .label("Approve Reader")
                            .style(ButtonStyle::Secondary);
                            let deny_btn = CreateButton::new(format!(
                                "dc_auth:deny:{}:{}",
                                comp.user.id, comp.user.name
                            ))
                            .label("Deny")
        ...
        if action == "approve" {
...

            {
                ...
                if let Some(existing) = settings_write
                    .users
                    .iter_mut()
                    .find(|u| u.discord_id.as_deref() == Some(target_user_id_str))
                {
                ...
                .create_response(
                    ...
                    CreateInteractionResponse::UpdateMessage(
                        CreateInteractionResponseMessage::new()
                            .content(format!(
                                "✅ Authorized **{}** (ID: `{}`) as **{}**.",
                                username, target_user_id_str, role
                            ))
                            .components(vec![]),
                    ),
                )
            ...
            let _ = comp
                .create_response(
                    ...
                    CreateInteractionResponse::UpdateMessage(
                        CreateInteractionResponseMessage::new()
                            .content(format!(
                                "❌ Access request for **{}** (ID: `{}`) was denied.",
                                username, target_user_id_str
                            ))
                            .components(vec![]),
                    ),
                )
        ...
        match handler.manager.get_torrent(hash, None).await {
            Ok(Some(torrent)) => match crate::s3::get_download_link(settings, &torrent).await {
                Ok(Some(url)) => {
                    let row = CreateActionRow::Buttons(vec![
                    ...
                    let _ = comp
                        .edit_response(
                            ...
                            EditInteractionResponse::new().content(
                                "❌ Download links are either disabled or not configured.",
                            ),
                        )
                        .await;
                }
        ...
        if user.role != "administrator" {
            let _ = comp
                .create_response(
                    ...
                    CreateInteractionResponse::Message(
                        CreateInteractionResponseMessage::new()
                            .content("Unauthorized. Only administrators can delete torrents.")
                            .ephemeral(true),
                    ),
                )
                .await;
        ...
        match res {
            Ok(_) => {
                let msg = format!(
                    "🗑️ Torrent deleted successfully{}.",
                    if delete_files {
                        " (including files)"
                    } else {
                        ""
