...
use crate::utils::{convert_size, convert_eta, format_progress, escape_markdown};
use crate::redis_client::RedisWrapper;
use crate::i18n::t;
use super::state::translate;

pub async fn send_menu(bot: &Bot, chat_id: ChatId, message_id: Option<MessageId>, user: &UserSettings, redis: &RedisWrapper) -> Result<(), String> {
    let role = &user.role;
...
pub async fn render_client_settings(bot: &Bot, chat_id: ChatId, message_id: MessageId, user: &UserSettings, manager: &dyn TorrentClient, settings: &Settings) -> Result<(), String> {
...
pub async fn render_torrent_details(bot: &Bot, chat_id: ChatId, message_id: MessageId, user: &UserSettings, torrent: &Torrent) -> Result<(), String> {
