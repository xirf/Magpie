...
use crate::db::{get_cached_presigned_url, cache_presigned_url};
...
pub fn get_s3_bucket(settings: &Settings, use_public: bool) -> Result<Box<Bucket>, String> {
    ...
    let access_key = s3_config.access_key.as_deref()
        .ok_or_else(|| "S3 access key is missing.".to_string())?;
    let secret_key = s3_config.secret_key.as_deref()
        .ok_or_else(|| "S3 secret key is missing.".to_string())?;
    let bucket_name = s3_config.bucket.as_deref()
    ...
    let region_name = s3_config.region.clone().unwrap_or_else(|| "us-east-1".to_string());
    let endpoint_opt = if use_public {
        s3_config.public_url.clone().or_else(|| s3_config.endpoint.clone())
    } else {
    ...
    let region = if let Some(ref endpoint) = endpoint_opt {
        ...
        region_name.parse().map_err(|e| format!("Invalid region: {:?}", e))?
    };
...
pub async fn generate_presigned_url(settings: &Settings, key: &str) -> Result<String, String> {
    ...
    let url = bucket.presign_get(key, expiry as u32, None)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn upload_folder_or_file_to_s3(settings: &Settings, local_path: &Path, key_prefix: &str) -> Result<(), String> {
    if !local_path.exists() {
        return Err(format!("Local path does not exist: {:?}", local_path));
    }
    
    if local_path.is_file() {
        let filename = local_path.file_name().and_then(|n| n.to_str()).unwrap_or("");
...
pub async fn get_download_link(settings: &Settings, torrent: &crate::torrent_client::Torrent) -> Result<Option<String>, String> {
    ...
    if local_enabled {
        ...
        let base_url = settings.local_server.base_url.as_deref().unwrap_or("http://localhost/");
        let base_url_trimmed = base_url.trim_end_matches('/');
        let torrent_name_encoded = crate::utils::percent_encode(&torrent.name);
        let url = format!("{}/download/{}/{}", base_url_trimmed, token, torrent_name_encoded);
        return Ok(Some(url));
    }
    ...
    if content_path.exists() {
        if content_path.is_dir() {
            ...
            key = content_path.file_name().and_then(|n| n.to_str()).unwrap_or(&torrent.name).to_string();
        }
    }
