...
pub(crate) fn get_resolved_user(settings: &Settings, telegram_id: i64) -> UserSettings {
    settings
        ...
        .cloned()
        .or_else(|| settings.users.iter().find(|u| u.user_id == 0).cloned())
...
pub(crate) async fn get_user_and_check_auth(
    bot: &Bot,
    settings: &Settings,
    msg: &Message,
) -> Option<UserSettings> {
    ...
    let is_authorized = settings
        .users
        .iter()
        .any(|u| u.user_id == telegram_id || u.user_id == 0);
    if !is_authorized {
        let keyboard =
