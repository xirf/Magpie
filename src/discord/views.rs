...
use crate::utils::{convert_eta, convert_size, format_progress};
...
pub async fn show_torrent_list(
    ...
    let dl_btn = CreateButton::new("dc_status:downloading")
        .label("⏳ Downloading")
        .style(if clean_filter == Some("downloading") {
            ButtonStyle::Primary
        } else {
    ...
    let comp_btn = CreateButton::new("dc_status:completed")
        .label("✔️ Completed")
        .style(if clean_filter == Some("completed") {
            ButtonStyle::Primary
        } else {
            ButtonStyle::Secondary
        });
    ...
    let pause_btn = CreateButton::new("dc_status:paused")
        .label("⏸️ Paused")
        .style(if clean_filter == Some("paused") {
            ButtonStyle::Primary
        } else {
            ButtonStyle::Secondary
        });
    ...
    let all_btn =
        CreateButton::new("dc_status:all")
            .label("📁 All")
            .style(if clean_filter.is_none() {
                ButtonStyle::Primary
            } else {
                ButtonStyle::Secondary
            });
    ...
    if torrents.is_empty() {
        ...
        let components = if clean_filter.is_some() {
            vec![filter_row]
        } else {
            vec![]
        };
...
pub async fn show_torrent_details(
    ...
    let embed = serenity::builder::CreateEmbed::new()
        ...
        .field(
            ...
            format!(
                "{} {}%",
                format_progress(torrent.progress, 20),
                progress_percent
            ),
            false,
        )
        ...
        .field(
            "Download Speed",
            format!("`{}/s`", convert_size(torrent.dlspeed)),
            true,
        )
        .field("ETA", format!("`{}`", convert_eta(torrent.eta)), true)
        .field(
            "Category",
            format!("`{}`", torrent.category.as_deref().unwrap_or("None")),
            true,
        )
