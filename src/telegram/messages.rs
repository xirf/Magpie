...
use super::state::{get_user_and_check_auth, translate, BotState};
use crate::utils::{
    convert_eta, convert_size, escape_markdown, extract_clean_magnet, extract_hash_from_magnet,
    format_progress, read_cpu_temp,
};
                        ...
                        if cmd_text.contains("info") {
                            match state.manager.get_torrent(&hash, None).await {
                                Ok(Some(t)) => {
                                    ...
                                    let _ = bot
                                        .send_message(msg.chat.id, details)
                                ...
                                Ok(Some(t)) => {
                                    if t.progress >= 1.0 {
                                        ...
                                        let _ = bot
                                            .send_message(
                                                msg.chat.id,
                                                "❌ Torrent is not fully completed yet.",
                                            )
                                            .reply_to_message_id(msg.id)
                                            .await;
                                    }
            ...
            if let Some(doc) = msg.document() {
                    ...
                    .is_some_and(|name| name.ends_with(".torrent"))
                {
                    ...
                    match bot.get_file(&doc.file.id).await {
                                ...
                                match state.manager.add_torrent(bytes, &filename, category).await {
                                    Ok(true) => {
                                        ...
                                        let new_torrent = after.iter().find(|t_after| {
                                            !before
                                                .iter()
                                                .any(|t_before| t_before.hash == t_after.hash)
                                        });
                                        let details_msg = if let Some(t) = new_torrent {
                                            let peers_line = match (t.num_seeds, t.num_peers) {
                                                ...
                                                (Some(seeds), None) => format!(
                                                    "\n*Peers:* {}",
                                                    escape_markdown(&format!("Seeds: {}", seeds))
                                                ),
                                                (None, Some(peers)) => format!(
                                                    "\n*Peers:* {}",
                                                    escape_markdown(&format!("Peers: {}", peers))
                                                ),
                                            };
                                        ...
                                        if let Ok(sent_msg) = bot
                                            .send_message(msg.chat.id, details_msg)
                                            .reply_to_message_id(msg.id)
                                            .parse_mode(teloxide::types::ParseMode::MarkdownV2)
                                            .await
                                        {
                        ...
                        Err(_) => {
                            let _ = bot
                                .send_message(msg.chat.id, "Failed to download attachment.")
                                .reply_to_message_id(msg.id)
                                .await;
                        }
                    }
    ...
    if action_val == "category_name" {
        ...
        state
            ...
            .set(
                ...
                &format!("category_dir_add#{}", text),
                None,
            )
            .await;
        let mut vars = HashMap::new();
        vars.insert("category_name".to_string(), text.to_string());
        let text_path = translate(
            &user,
            "Please, send the path for the category {category_name}",
            Some(&vars),
        );
        let _ = bot
            .send_message(msg.chat.id, text_path)
            .reply_to_message_id(msg.id)
            .await;
        ...
        let _ = bot
            .send_message(msg.chat.id, "Category created successfully!")
            .await;
        state
            .redis
            .set(&format!("action:{}", user_id), "", None)
            .await;
        ...
        let _ = bot
            .send_message(msg.chat.id, "Category path modified successfully!")
            .await;
        state
            .redis
            .set(&format!("action:{}", user_id), "", None)
        ...
        if let Some(magnet_link) = extract_clean_magnet(text) {
            ...
            match state.manager.add_magnet(&magnet_link, category).await {
                                ...
                                for _ in 0..15 {
                                    ...
                                    if let Ok(Some(t)) = manager.get_torrent(&h, None).await {
                                        if t.size > 0 && t.name != h && !t.name.is_empty() {
                                            ...
                                            let _ = bot_clone
                                                .edit_message_text(chat_id, msg_id, updated_msg)
                                                .parse_mode(teloxide::types::ParseMode::MarkdownV2)
                                                .await;
                                            break;
                                        }
                ...
                Ok(false) => {
                    ...
                    let _ = bot
                        .send_message(msg.chat.id, "❌ Failed to add magnet link.")
                        .reply_to_message_id(msg.id)
                        .reply_markup(keyboard)
                        .await;
                }
